...
// .wrangler/tmp/bundle-SqPJEZ/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-SqPJEZ/middleware-insertion-facade.js
init_modules_watch_stub();
...
function toCamelCase(input) {
  const words = input.replace(/['\u2019]/g, "").match(/[\da-z]+|[A-Z]+(?![a-z])|[A-Z][\da-z]+/g) ?? [];
  return words.reduce((acc2, word, i) => {
    const formattedWord = i === 0 ? word.toLowerCase() : `${word[0].toUpperCase()}${word.slice(1)}`;
    return acc2 + formattedWord;
  }, "");
}
...
var voicemailBoxes = pgTable("voicemail_boxes", {
  id: serial().primaryKey().notNull(),
  accountId: integer("account_id").default(0).notNull(),
  domain: varchar({ length: 64 }).default("").notNull(),
  ...
  messageCapacity: integer("message_capacity").default(1e3).notNull(),
...
var subscriber = pgTable("subscriber", {
  id: serial().primaryKey().notNull(),
  accountId: integer("account_id").default(0).notNull(),
...
var extensions = pgTable("extensions", {
  id: serial().primaryKey().notNull(),
  accountId: integer("account_id").default(0).notNull(),
...
var voicemailMessages = pgTable("voicemail_messages", {
  id: serial().primaryKey().notNull(),
  accountId: integer("account_id").default(0).notNull(),
...
var dbaliases = pgTable("dbaliases", {
  id: serial().primaryKey().notNull(),
...
var dispatcher = pgTable("dispatcher", {
  id: serial().primaryKey().notNull(),
...
var dialogVars = pgTable("dialog_vars", {
  id: serial().primaryKey().notNull(),
  hashEntry: integer("hash_entry").notNull(),
  hashId: integer("hash_id").notNull(),
...
var locationAttrs = pgTable("location_attrs", {
  id: serial().primaryKey().notNull(),
  ruid: varchar({ length: 64 }).default("").notNull(),
  username: varchar({ length: 64 }).default("").notNull(),
  domain: varchar({ length: 64 }).default(sql`NULL`),
...
var lcrRuleTarget = pgTable("lcr_rule_target", {
  id: serial().primaryKey().notNull(),
  lcrId: smallint("lcr_id").notNull(),
...
var domainAttrs = pgTable("domain_attrs", {
  id: serial().primaryKey().notNull(),
...
var usrPreferences = pgTable("usr_preferences", {
  id: serial().primaryKey().notNull(),
...
accountsRoute.post("/", async (c) => {
  const body = await c.req.json();
  const { extAccountId, domain: domain2 } = body;
  try {
    ...
    console.error(error);
    return c.json({ error: error instanceof Error ? error.message : error?.message || "Unknown error" }, 500);
  }
});
accountsRoute.put("/:id", async (c) => {
  const id = parseInt(c.req.param("id"));
  const body = await c.req.json();
  const { extAccountId, domain: domain2, isActive } = body;
  if (!extAccountId)
...
var accounts_default = accountsRoute;

// src/server/routes/subscribers.ts
init_modules_watch_stub();
import crypto2 from "crypto";
var subscribersRoute = new Hono2();
subscribersRoute.get("/", async (c) => {
  const subscribersData = await db.select().from(subscriber);
  return c.json({ subscribers: subscribersData });
});
subscribersRoute.post("/", async (c) => {
  const body = await c.req.json();
  const { accountId, extensionId, defaultDid, username, domain: domain2, password } = body;
  if (!accountId)
  ...
  const ha1 = crypto2.createHash("md5").update(password).digest("hex");
  const ha1b = crypto2.createHash("md5").update(`${username}:${domain2}`).digest("hex");
  try {
    const subscriber2 = await db.insert(subscriber).values({
      accountId,
      extensionId,
      defaultDid,
      username,
    ...
    return c.json({ subscriber: subscriber2[0] }, 201);
  } catch (error) {
    console.error(error);
    return c.json({ error: error instanceof Error ? error.message : error?.message || "Unknown error" }, 500);
  }
});
var subscribers_default = subscribersRoute;
...
function wrapExportedHandler(worker) {
  ...
  return {
    ...
    fetch(request, env, ctx) {
      const dispatcher2 = /* @__PURE__ */ __name(function(type, init) {
      ...
      return __facade_invoke__(request, env, ctx, dispatcher2, fetchDispatcher);
    }
  };
}
