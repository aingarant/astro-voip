  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-slate-900">Accounts Management</h2>
          <p className="text-slate-500 font-medium mt-1">Configure and monitor external SIP providers and domain routing.</p>
        </div>
        <button 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-50 bg-slate-50/30">
                ...
                <th className="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Account Identifier</th>
                <th className="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Domain Path</th>
                ...
                <tr>
                  <td colSpan={5} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <span className="text-sm font-bold text-slate-400 uppercase tracking-widest">Loading Infrastructure Data...</span>
                    </div>
                  </td>
                </tr>
                  ...
                  <tr key={acc.id} className="group hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-bold text-slate-900 tracking-tight">{acc.extAccountId}</span>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <span className="text-sm font-medium text-slate-600 font-mono bg-slate-50 px-3 py-1 rounded-lg border border-slate-100">{acc.domain}</span>
                    </td>
                    <td className="px-8 py-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8 animate-in fade-in zoom-in duration-300">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl shadow-indigo-100 overflow-hidden border border-slate-100">
            <div className="px-10 py-8 border-b border-slate-50 flex justify-between items-center">
              <div>
                <h3 className="text-xl font-bold text-slate-900">{editingId ? 'Configure Account' : 'New SIP Account'}</h3>
                <p className="text-xs text-slate-400 font-medium mt-1">Provide the necessary credentials for the account.</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-8">
              <div className="space-y-6">
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Account Identifier</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">SIP Domain / Gateway</label>
                  <input
                    type="text"
                    required
                ...
                {editingId && (
                  <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <div>
                      <span className="block text-[10px] font-bold text-slate-900 uppercase tracking-widest">Operational Status</span>
                      <span className="text-[10px] text-slate-400 font-medium">Toggle account connectivity</span>
                    </div>
              ...
              <div className="pt-4 flex gap-4">
                ...
                <button 
                  type="submit" 
                  className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl text-sm font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all disabled:opacity-50 disabled:shadow-none"
